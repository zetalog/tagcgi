...
unsigned int re_max_failures = 40000;
...
struct range_table_work_area
{
  ...
  unsigned int allocated;	/* allocated size for work area in bytes.  */
	...
	case '[':
	  {
		...
		if (SINGLE_BYTE_CHAR_P (c))
		  /* ... into bitmap.  */
		  {
		    ...
		    unsigned int range_start = c, range_end = c1;
	  ...
	  DEBUG_PRINT2 ("EXECUTING maybe_pop_jump %d.\n", mcnt);
	  {
		...
		if ((re_opcode_t) p1[3] == exactn)
			 ...
			 || (re_opcode_t) p1[3] == charset_not)
		  {
		    ...
		    if (SINGLE_BYTE_CHAR_P (c))
		      {
			if (c < (unsigned)(CHARSET_BITMAP_SIZE (&p1[3]) * BYTEWIDTH)
		...
		if ((re_opcode_t) p1[3] == exactn)
		  {
		    ...
		    if (SINGLE_BYTE_CHAR_P (c))
		      {
			if (c < (unsigned) (CHARSET_BITMAP_SIZE (p2) * BYTEWIDTH)
			    && (p2[2 + c / BYTEWIDTH]
				& (1 << (c % BYTEWIDTH))))
			  not = !not;
		      }
